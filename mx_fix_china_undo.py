...
# ==========================================
# ⚙️ 配置路径 (请确认是你的 Web 仓库路径)
# ==========================================
repo_path = r"F:\mx_radar_web"  # 👈 如果你的路径变了请修改这里
history_file = os.path.join(repo_path, "history_1d_china.json")
consensus_file = os.path.join(repo_path, "consensus_1d_china.json")
...
def fix_and_push():
    ...
    print("✅ history_1d_china.json 已成功复原。")
    ...
    if len(history_data) > 0:
        ...
        print(f"✅ consensus_1d_china.json 已重置回: 🕰️ {last_valid_snapshot.get('timestamp')}")
    else:
    ...
    try:
        os.chdir(repo_path)
        subprocess.run(["git", "add", "history_1d_china.json", "consensus_1d_china.json"], check=True)
        subprocess.run(["git", "commit", "-m", "⚡ Revert: Undo accidental duplicate snapshot"], check=True)
